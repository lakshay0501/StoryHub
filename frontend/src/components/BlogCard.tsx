...
export const BlogCard = ({
    ...
    <div className="p-4 m-1 mb-2 border-b border-slate-200 pb-4 w-screen max-w-screen-lg cursor-pointer">
      <div className="flex items-center">
        ...
        <div className="pl-2 font-thin flex justify-center flex-col">{publishedDate}</div>
      </div>
...
export function Avatar({name,size="small"}:{name:string,size:"small" | "big"}){
    return <div>
        <div className={`relative inline-flex items-center justify-center  overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600 ${size==="small" ? "w-6 h-6" : "w-10 h-10"}`}>
         <span className={`${size==="small" ? "text-xs" : "text-md"} text-xs font-thin text-gray-600 dark:text-gray-300`}>{name[0]}</span>
       </div>
    </div>
}